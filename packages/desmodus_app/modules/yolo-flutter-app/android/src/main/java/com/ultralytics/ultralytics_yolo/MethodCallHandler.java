...
public class MethodCallHandler implements MethodChannel.MethodCallHandler {
    ...
    public void onMethodCall(@NonNull MethodCall call, @NonNull MethodChannel.Result result) {
        ...
        switch (method) {
            ...
            case "setLensDirection":
                ...
                break;
            case "setFlashlightValue":
                setFlashlightValue(call, result);
                break;
            case "takePicture":
    ...
    private void setFlashlightValue(MethodCall call, MethodChannel.Result result) {
        Object flashlightValue = call.argument("value");
        if (flashlightValue != null) {
            final boolean value = (boolean) flashlightValue;
            cameraPreview.setFlashlightValue(value);
        }
    }

    private void takePicture(MethodCall call, MethodChannel.Result result) {
        result.success(cameraPreview.takePicture());
    }
...
//        ncnnCameraPreview.closeCamera();
    ...
    private void pauseLivePrediction(MethodCall call, MethodChannel.Result result) {
        ...
        if (directionObject != null) {
            final int direction = (int) directionObject;
            cameraPreview.pauseLivePrediction(direction);
        }
    }
    ...
    private void resumeLivePrediction(MethodCall call, MethodChannel.Result result) {
        ...
        if (directionObject != null) {
            final int direction = (int) directionObject;
            cameraPreview.resumeLivePrediction(direction);
        }
    }
